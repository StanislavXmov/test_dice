...
type EventTable = 'unification' | 'intersection';

const information = {
  '6': 'Бросили два шестигранных кубика. На первом кубике выпала двойка, на другом — тройка. Выделите в таблице соответсвующие ячейки.',
  '8': 'Бросили два восьмигранных кубика. На первом кубике выпала двойка, на другом — тройка. Выделите в таблице соответсвующие ячейки..',
  '12': 'Бросили два 12-гранных кубика. На первом кубике выпала двойка, на другом — тройка. Выделите в таблице соответсвующие ячейки..',
}
...
const TableType1 = ({ tableView }: {tableView: ViewTable}) => {
  const selected = useTableType2(s => s.selected);
  const add = useTableType2(s => s.add);
  const addMore = useTableType2(s => s.addMore);
  const removeIds = useTableType2(s => s.removeIds);
...
const Table = () => {
  ...
  const [eventTable, setEventTable] = useState<EventTable>('unification');
  const [disabled, setDisabled] = useState(false);

  const [valueAnswer, setValueAnswer] = useState(false);

  const type = useTableType2(s => s.type);
  const setType = useTableType2(s => s.setType);
  const selected = useTableType2(s => s.selected);
  ...
  const selectTableEventHandler = (e: ChangeEvent<HTMLSelectElement>) => {
    setEventTable(e.target.value as EventTable);
  }
    ...
    <>
      <div className={styles.wrapper}>
        ...
        <div className={
          ...
          <div className={styles.infoColorFrapper}>
            <span className={styles.label}>Цвета:</span>
            <span className={`${styles.label} ${styles.yellow}`}>первое условие — один клик</span>
            <span className={`${styles.label} ${styles.blue}`}>второе условие — два клика</span>
            <span className={`${styles.label} ${styles.green}`}>пересечение условий</span>
          </div>
          <label className={styles.label} htmlFor="events">
            <select
              id='events'
              className={styles.tableViewSelect}
              onChange={selectTableEventHandler}
            >
              <option value={'unification'}>По объединению условий</option>
              <option value={'intersection'}>По пересечению условий</option>
            </select>
          </label>
          <div>
            {selected.length === 0 && (
